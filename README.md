...
OS のスタートアッププログラムの代替として、登録したアプリを遅延起動する Windows 用ユーティリティです。このアプリ自体を **タスク スケジューラ** に登録し、そこから各アプリを順に起動します（スタートアップへの登録も可能ですが、タスク スケジューラの方が速く・確実に起動するため推奨します）。
...
スタートアップで同時に起動するアプリが増えると、起動に失敗するものや、起動はしていてもタスクトレイにアイコンが出ないものが出てくることがあります。遅延起動で起動タイミングをずらすことで、こうした不具合を抑えられます。

本アプリは **タスク スケジューラへの登録を推奨**します。スタートアップ経由より速く・確実に起動します。Setting に登録したアプリを Delay 付きで順次起動します。

> スタートアップからの起動に失敗する場合も、タスク スケジューラからの起動を試してください。
